...
export default function CollectionClientGrid({ initialProducts, categories }: ClientGridProps) {
  ...
  const filtered = active === "All" 
    ? initialProducts 
    : initialProducts.filter(p => p.category.toLowerCase() === active.toLowerCase());
                ...
                <li key={cat}>
                  ...
                  >
                    ...
                    <span style={{ fontSize: "0.6rem", color: active === cat ? "var(--amber)" : "rgba(247,243,238,0.2)", fontWeight: 400 }}>
                      {cat === "All" ? initialProducts.length : initialProducts.filter(p => p.category.toLowerCase() === cat.toLowerCase()).length}
                    </span>
                  </button>
                </li>
              ))}
